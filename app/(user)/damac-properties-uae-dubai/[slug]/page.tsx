...
export async function generateStaticParams() {
  const query = groq`
    *[_type=="properties" && references(*[_type == "developer" && title == "Damac Properties"]._id)]{
      slug
    }`;
...
export default async function FeaturedProperty({
  ...
  const query = groq`
  *[_type == "properties" && references(*[_type == "developer" && title == "Damac Properties"]._id) && slug.current == $slug][0] {
    _id,
    title,
    "slug": slug.current,
