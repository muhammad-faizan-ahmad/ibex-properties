...
export async function generateStaticParams() {
  const query = groq`
    *[_type=="properties" && references(*[_type == "category" && title == "Rent"]._id)] {
      slug
    }`;
...
export default async function RentProperty({
  params: { slug },
}: {
  params: { slug: string };
}) {
  const query = groq`
  *[_type == "properties" && references(*[_type == "category" && title == "Rent"]._id) && slug.current == $slug][0] {
    _id,
    title,
    "slug": slug.current,
