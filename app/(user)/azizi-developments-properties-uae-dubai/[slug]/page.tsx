...
export async function generateStaticParams() {
  const query = groq`
    *[_type=="properties" && references(*[_type == "developer" && title == "Azizi Developments"]._id) ]{
      slug
    }`;
...
export default async function FeaturedProperty({
  ...
  const query = groq`
  *[_type == "properties" && references(*[_type == "developer" && title == "Azizi Developments"]._id) && slug.current == $slug][0] {
    _id,
    title,
    "slug": slug.current,
