...
export async function generateStaticParams() {
  const query = groq`
    *[_type=="properties" && references(*[_type == "developer" && title == "Riviera Properties"]._id)]{
      slug
    }`;
...
export default async function RivieraProperty({
  params: { slug },
}: {
  params: { slug: string };
}) {
  const query = groq`
  *[_type == "properties" && references(*[_type == "developer" && title == "Riviera Properties"]._id) && slug.current == $slug][0] {
    _id,
    title,
    "slug": slug.current,
